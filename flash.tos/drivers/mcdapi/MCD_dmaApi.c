...
)
{
    ...
    u32 *realFuncArray;
#endif
		volatile TaskTableEntry *MCD_taskTableChannel;
...
#ifdef MCD_DEBUG
    Cconws("startDma:Setting up params\r\n");
#endif
   /* Set us up for task-wise priority.  We don't technically need to do this on every start, but
...
#ifdef MCD_PROGQUERY /* Not sure what we need to keep here rtm TBD */
    ...
    MCD_remVariants.remXferSize[channel] = xferSize;
		...
		MCD_taskTableChannel = &MCD_taskTable[channel];
    cSave = (int*)(MCD_taskTableChannel->contextSaveSpace) + CSAVE_OFFSET + CURRBD;

#ifdef MCD_INCLUDE_EU /* may move this to EU specific calls */
    realFuncArray = (u32 *) (MCD_taskTableChannel->FDTandFlags & 0xffffff00);
   ...
   /* Write the initiator field in the TCR, and also set the initiator-hold
    ...
    MCD_taskTableChannel->FDTandFlags &= ~MCD_TT_FLAGS_MASK;
    MCD_taskTableChannel->FDTandFlags |= (MCD_TT_FLAGS_MASK & flags);

    if (flags & MCD_FECTX_DMA)
    {
        /* TDTStart and TDTEnd */
        MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_FECTX].TDTstart;
        MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_FECTX].TDTend;
        MCD_startDmaENetXmit(srcAddr, srcAddr, destAddr, MCD_taskTable, channel);
    }
    else if (flags & MCD_FECRX_DMA)
    {
        /* TDTStart and TDTEnd */
        MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_FECRX].TDTstart;
        MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_FECRX].TDTend;
    ...
    else if(flags & MCD_SINGLE_DMA)
    {
        ...
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[SRCPTR + CSAVE_OFFSET] = (int)srcAddr;
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[DESTPTR + CSAVE_OFFSET] = (int)destAddr;
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[DCOUNT + CSAVE_OFFSET] = 0;
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[CURRBD + CSAVE_OFFSET] =
...
#endif
           /* TDTStart and TDTEnd */
           MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_SINGLENOEU].TDTstart;
           MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_SINGLENOEU].TDTend;
        ...
        {
           /* TDTStart and TDTEnd */
           MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_SINGLEEU].TDTstart;
           MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_SINGLEEU].TDTend;
    ...
    { /* chained DMAS */
        ...
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[SRCPTR + CSAVE_OFFSET]
          = (int)((MCD_bufDesc*) srcAddr)->srcAddr;
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[DESTPTR + CSAVE_OFFSET]
...
#endif
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[DCOUNT + CSAVE_OFFSET] = 0;
        ((volatile int *)MCD_taskTableChannel->contextSaveSpace)[CURRBD + CSAVE_OFFSET] = (u32) srcAddr;
#ifdef MCD_INCLUDE_EU
        if( funcDesc == MCD_FUNC_NOEU1 || funcDesc == MCD_FUNC_NOEU2)
        {
#endif
          /*TDTStart and TDTEnd*/
          MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_CHAINNOEU].TDTstart;
          MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_CHAINNOEU].TDTend;
        ...
        {
          /*TDTStart and TDTEnd*/
          MCD_taskTableChannel->TDTstart = MCD_modelTaskTable[TASK_CHAINEU].TDTstart;
          MCD_taskTableChannel->TDTend = MCD_modelTaskTable[TASK_CHAINEU].TDTend;
