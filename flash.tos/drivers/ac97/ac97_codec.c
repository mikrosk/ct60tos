...
#include "pcm.h"
#include "tlv.h"
#undef CONFIG_AC97_TUNE
#undef CONFIG_AC97_PATCH
#include "ac97_codec.h"
#include "ac97_id.h"

#define ARRAY_SIZE(x) (sizeof(x)/sizeof(x[0]))
#define msecs_to_jiffies(timeout) (timeout/5) 

extern void udelay(long usec);
extern void ltoa(char *buf, long n, unsigned long base);

static int enable_loopback; // option
...
{
	if(!ac97->reg_accessed[reg])
	{
		ac97->regs[reg] = ac97->bus->ops->read(ac97, reg);
		// ac97->reg_accessed[reg] = 1;
	}
...
//		mutex_lock(&ac97->reg_mutex);
		...
		/* select single codec */
		ac97->bus->ops->write(ac97, AC97_AD_SERIAL_CFG, (cfg & ~0x7000) | ac97->spec.ad18xx.unchained[codec] | ac97->spec.ad18xx.chained[codec]);
		/* update PCM bits */
		ac97->bus->ops->write(ac97, AC97_PCM, new);
		/* select all codecs */
		ac97->bus->ops->write(ac97, AC97_AD_SERIAL_CFG, cfg | 0x7000);
//		mutex_unlock(&ac97->reg_mutex);
	}
...
static int snd_ac97_info_enum_double(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
static int snd_ac97_get_enum_double(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int snd_ac97_put_enum_double(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	struct ac97_enum *e = (struct ac97_enum *)kcontrol->private_value;
	unsigned short val;
...
static int snd_ac97_page_save(struct snd_ac97 *ac97, int reg, struct snd_kcontrol *kcontrol)
{
	...
	if((kcontrol->private_value & (1<<25))
	 && (ac97->ext_id & AC97_EI_REV_MASK) >= AC97_EI_REV_23 && (reg >= 0x60 && reg < 0x70))
	{
...
static int snd_ac97_info_volsw(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
static int snd_ac97_get_volsw(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	...
	int page_save = snd_ac97_page_save(ac97, reg, kcontrol);
...
static int snd_ac97_put_volsw(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int reg = kcontrol->private_value & 0xff;
	int shift = (kcontrol->private_value >> 8) & 0x0f;
...
static struct snd_kcontrol *snd_ac97_cnew(const struct snd_kcontrol_new *_template, struct snd_ac97 *ac97)
{
...
static int snd_ac97_cmute_new_stereo(struct snd_card *card, char *name, int reg,
				     int check_stereo, int check_amix, struct snd_ac97 *ac97)
{
			     ...
			     unsigned int hi_max, struct snd_ac97 *ac97)
{
	...
	if(err < 0)
		return err;
	snd_ac97_write_cache(ac97, reg, (snd_ac97_read(ac97, reg) & 0x8080) | lo_max | (hi_max << 8));
	return 0;
}
				    ...
				    struct snd_ac97 *ac97)
{
	...
	if(snd_ac97_try_bit(ac97, reg, 15))
	{
		strcpy(name, pfx);
		strcat(name, " Switch");
		if((err = snd_ac97_cmute_new_stereo(card, name, reg, check_stereo, check_amix, ac97)) < 0)
			return err;
	}
...
static int snd_ac97_mixer_build(struct snd_ac97 * ac97)
{
	...
	if(snd_ac97_try_volume_mix(ac97, AC97_MASTER))
	{
		if(ac97->flags & AC97_HAS_NO_MASTER_VOL)
			err = snd_ac97_cmute_new(card, "Master Playback Switch", AC97_MASTER, 0, ac97);
		else
			err = snd_ac97_cmix_new(card, "Master Playback", AC97_MASTER, 0, ac97);
		if(err < 0)
			return err;
	}
	ac97->regs[AC97_CENTER_LFE_MASTER] = 0x8080;
	/* build center controls */
	if((snd_ac97_try_volume_mix(ac97, AC97_CENTER_LFE_MASTER)) && !(ac97->flags & AC97_AD_MULTI))
	{
	...
	/* build LFE controls */
	if((snd_ac97_try_volume_mix(ac97, AC97_CENTER_LFE_MASTER+1)) && !(ac97->flags & AC97_AD_MULTI))
	{
	 ...
	 && !(ac97->flags & AC97_AD_MULTI))
	{
		...
		if((err = snd_ac97_cmix_new_stereo(card, "Surround Playback", AC97_SURROUND_MASTER, 1, 0, ac97)) < 0)
			return err;
	}
	/* build headphone controls */
	if(snd_ac97_try_volume_mix(ac97, AC97_HEADPHONE))
	{
		if ((err = snd_ac97_cmix_new(card, "Headphone Playback", AC97_HEADPHONE, 0, ac97)) < 0)
			return err;
	}	
	/* build master mono controls */
	if(snd_ac97_try_volume_mix(ac97, AC97_MASTER_MONO))
	{
		if((err = snd_ac97_cmix_new(card, "Master Mono Playback", AC97_MASTER_MONO, 0, ac97)) < 0)
			return err;
	}
	...
	if(!(ac97->flags & AC97_HAS_NO_PC_BEEP)
	 && ((ac97->flags & AC97_HAS_PC_BEEP) || snd_ac97_try_volume_mix(ac97, AC97_PC_BEEP)))
	{
		for(idx = 0; idx < 2; idx++)
		{
		...
		snd_ac97_write_cache(ac97, AC97_PC_BEEP, snd_ac97_read(ac97, AC97_PC_BEEP) | 0x801e);
	}
	...
	if(!(ac97->flags & AC97_HAS_NO_PHONE))
	{
		...
		{
			if((err = snd_ac97_cmix_new(card, "Phone Playback", AC97_PHONE, 1, ac97)) < 0)
				return err;
		}
	}
	/* build MIC controls */
	if(!(ac97->flags & AC97_HAS_NO_MIC))
	{
		...
		{
			if((err = snd_ac97_cmix_new(card, "Mic Playback", AC97_MIC, 1, ac97)) < 0)
				return err;
	...
	{
		if((err = snd_ac97_cmix_new(card, "Line Playback", AC97_LINE, 1, ac97)) < 0)
			return err;
	}
	/* build CD controls */
	if(!(ac97->flags & AC97_HAS_NO_CD))
	{
		...
		{
			if((err = snd_ac97_cmix_new(card, "CD Playback", AC97_CD, 1, ac97)) < 0)
				return err;
		}
	}
	...
	if(!(ac97->flags & AC97_HAS_NO_VIDEO))
	{
		...
		{
			if((err = snd_ac97_cmix_new(card, "Video Playback", AC97_VIDEO, 1, ac97)) < 0)
				return err;
		}
	}
	/* build Aux controls */
	if(!(ac97->flags & AC97_HAS_NO_AUX))
	{
		...
		{
			if((err = snd_ac97_cmix_new(card, "Aux Playback", AC97_AUX, 1, ac97)) < 0)
				return err;
		}
	}
	/* build PCM controls */
	...
	else
	{
		if(!(ac97->flags & AC97_HAS_NO_STD_PCM))
		{
			if(ac97->flags & AC97_HAS_NO_PCM_VOL)
				err = snd_ac97_cmute_new(card, "PCM Playback Switch", AC97_PCM, 0, ac97);
			else
				err = snd_ac97_cmix_new(card, "PCM Playback", AC97_PCM, 0, ac97);
			if(err < 0)
				return err;
		}
	}
	...
	if(!(ac97->flags & AC97_HAS_NO_REC_GAIN))
	{
		...
		{
			err = snd_ac97_cmute_new(card, "Capture Switch", AC97_REC_GAIN, 0, ac97);
			if (err < 0)
				return err;
		}
...
static void snd_ac97_determine_rates(struct snd_ac97 *ac97, int reg, int shadow_reg, unsigned int *r_result)
{
	...
	if(ac97->bus->no_vra)
	{
		*r_result = SNDRV_PCM_RATE_48000;
		if((ac97->flags & AC97_DOUBLE_RATE) && (reg == AC97_PCM_FRONT_DAC_RATE))
...
static const struct ac97_codec_id *look_for_codec_id(const struct ac97_codec_id *table, unsigned int id)
{
...
void snd_ac97_get_name(struct snd_ac97 *ac97, unsigned int id, char *name, int modem)
{
	...
	if(ac97 && pid->patch)
	{
		if((modem && (pid->flags & AC97_MODEM_PATCH)) || (!modem && !(pid->flags & AC97_MODEM_PATCH)))
			pid->patch(ac97);
	} 
...
//	};
	...
	bus = (struct snd_ac97_bus *)Mxalloc(sizeof(*bus), 2);
	if (bus == NULL)
		return -ENOMEM;
	bus->card = card;
	bus->num = num;
...
//	};
	...
	*rac97 = NULL;
	card = bus->card;
	ac97 = (struct snd_ac97 *)Mxalloc(sizeof(*ac97), 2);
	if(ac97 == NULL)
		return -ENOMEM;
...
//		if (err < 0) {
//			printk("AC'97 %d does not respond - RESET\n", ac97->num);
...
__access_ok:
	ac97->id = snd_ac97_read(ac97, AC97_VENDOR_ID1) << 16;
	ac97->id |= snd_ac97_read(ac97, AC97_VENDOR_ID2);
	if(!(ac97->scaps & AC97_SCAP_DETECT_BY_VENDOR) && (ac97->id == 0x00000000 || ac97->id == 0xffffffff))
	{
//		printk("AC'97 %d access is not valid [0x%x], removing mixer.\n", ac97->num, ac97->id);
		snd_ac97_free(ac97);
		return -EIO;
	}
	pid = look_for_codec_id(snd_ac97_codec_ids, ac97->id);
...
//		if(!(ac97->scaps & (AC97_SCAP_SKIP_AUDIO|AC97_SCAP_SKIP_MODEM)))
			display_string("AC'97 access error (not audio or modem codec)\r\n");
//			printk("AC'97 %d access error (not audio or modem codec)\n", ac97->num);
		snd_ac97_free(ac97);
		return -EACCES;
	}
	...
	if(ac97_is_audio(ac97))
	{
		...
		end_time = *_hz_200 + msecs_to_jiffies(100);
		do
		{
...
//			schedule_timeout_uninterruptible(1);
		...
		while(*_hz_200 < end_time);
		display_string("AC'97 analog subsections not ready\r\n");
//		printk("AC'97 %d analog subsections not ready\n", ac97->num);
	}
	/* FIXME: add powerdown control */
...
//			schedule_timeout_uninterruptible(1);
		...
		while(*_hz_200 < end_time);
		display_string("MC'97 converters and GPIO not ready\r\n");
//		printk("MC'97 %d converters and GPIO not ready (0x%x)\n", ac97->num, snd_ac97_read(ac97, AC97_EXTENDED_MSTATUS));
	}
...
static void update_power_regs(struct snd_ac97 *ac97)
{
	...
	for(i = 0; i < PWIDX_SIZE; i++)
	{
		...
		snd_ac97_update_bits(ac97, power_regs[i].power_reg, power_regs[i].mask, bits);
	}
	if(!power_up)
	{
		if(!(ac97->regs[AC97_POWERDOWN] & AC97_PD_PR2))
