...
xBlockLink *pxFirstFreeBlock;														\
  ...
  memset(__FREERTOS_BASE, 0, (size_t)__FREERTOS_SIZE);               \
	xStart.pxNextFreeBlock = ( void * ) __FREERTOS_BASE;								\
	xStart.xBlockSize = ( size_t ) 0;												\
																					\
	/* xEnd is used to mark the end of the list of free blocks. */					\
	xEnd.xBlockSize = (size_t)__FREERTOS_SIZE;										\
	xEnd.pxNextFreeBlock = NULL;													\
																					\
	/* To start with there is a single free block that is sized to take up the		\
	entire heap space. */															\
	pxFirstFreeBlock = ( void * ) __FREERTOS_BASE;										\
	pxFirstFreeBlock->xBlockSize = (size_t)__FREERTOS_SIZE;							\
...
void *pvReturn = NULL;
	...
	vTaskSuspendAll();
	{
		...
		if( ( xWantedSize > 0 ) && ( xWantedSize < (size_t)__FREERTOS_SIZE ) )
		{
