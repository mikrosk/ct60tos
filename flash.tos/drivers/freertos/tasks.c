...
#if( HAVE_USP == 1 )
	unsigned portBASE_TYPE	uxSuper;			/*< Task runs in super or user mode. */
#endif
  long magic;
} tskTCB;
 ...
 */
	...
	#define tskSIZE_OF_EACH_TRACE_LINE			( ( unsigned portLONG ) ( sizeof( unsigned portLONG ) * 3 ) )
	...
	{																								\
		...
		{																							\
			...
			{																						\
				...
				{																					\
					unsigned portLONG *p = ( unsigned portLONG * ) pcTraceBuffer;	\
					uxPreviousTask = pxCurrentTCB->uxTCBNumber;										\
					*p++ = ( unsigned portLONG ) xTickCount;											\
					*p++ = ( unsigned portLONG ) uxPreviousTask;									\
					*p++ = ( unsigned portLONG ) uxPortReadTimer();								\
					pcTraceBuffer = ( volatile signed portCHAR * ) p;							\
				}																					\
				else																				\
				{																					\
					xTracing = pdFALSE;																\
 ...
 */
	...
	static void prvListTaskWithinSingleList( void *pxListRet, xList *pxList, signed portCHAR cStatus );
...
static unsigned portBASE_TYPE uxTaskNumber = 0; /*lint !e956 Static is deliberate - this is guarded before use. */
	...
	{		
...
#if( HAVE_USP == 1 )
		portSTACK_TYPE *pxTopOfUserStack = NULL;
#endif
		...
		#if portSTACK_GROWTH < 0
		{
			...
			{
				pxTopOfUserStack = pxNewTCB->pxStack + ( pxNewTCB->usStackDepth - 1 );
				pxTopOfStack = pxNewTCB->pxStack + ( pxNewTCB->usStackDepth/2 - 1 );
			}
		}
		#else
		{
			...
			{
				pxTopOfUserStack = pxNewTCB->pxStack;	
				pxTopOfStack = pxNewTCB->pxStack + pxNewTCB->usStackDepth/2;
			}
		}
		#endif
#else
...
/*-----------------------------------------------------------*/
	...
	{
		unsigned portBASE_TYPE uxQueue, uxNumberOfTasks, uxCount, uxNumber;
		struct listTCB
		{
...
#if( HAVE_USP == 1 )
			unsigned portBASE_TYPE	uxSuper;
#endif
		...
		uxNumberOfTasks = uxCurrentNumberOfTasks;
		pxListTCB = ( struct listTCB * ) pvPortMalloc( ( uxNumberOfTasks + 1 ) * sizeof( struct listTCB ) );
		if( pxListTCB != NULL )
		{
			memset( pxListTCB, 0, uxNumberOfTasks * sizeof( struct listTCB ) );
			...
			do
			{
				...
				{
					prvListTaskWithinSingleList( pxListTCB, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), tskREADY_CHAR );			
				}
			}
			...
			{
				prvListTaskWithinSingleList( pxListTCB, ( xList * ) pxDelayedTaskList, tskBLOCKED_CHAR );
			}

			if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
			{
				prvListTaskWithinSingleList( pxListTCB, ( xList * ) pxOverflowDelayedTaskList, tskBLOCKED_CHAR );
			}

			if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
			{
				prvListTaskWithinSingleList( pxListTCB, ( xList * ) &xTasksWaitingTermination, tskDELETED_CHAR );
			}

			if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
			{
				prvListTaskWithinSingleList( pxListTCB, ( xList * ) &xSuspendedTaskList, tskSUSPENDED_CHAR );
			}

			xTaskResumeAll();

			uxCount = uxNumber = 0;
			while( uxCount < uxNumberOfTasks) /* sort by # */
#if( HAVE_USP == 1 )
			{
				struct listTCB *pxList = pxListTCB;
				unsigned portSHORT usStackRemaining, usUserStackRemaining, usStackDepth;

				/* Write the details of all the TCB's in pxList into the buffer. */
				while( pxList->pxTCB != NULL )
				{
					if(uxNumber ==  pxList->uxTCBNumber)
					{
			         ...
			         pxList->cStatus == tskREADY_CHAR ? "Ready" : 
			          pxList->cStatus == tskBLOCKED_CHAR ? "Blocked" :
			           pxList->cStatus == tskSUSPENDED_CHAR ? "Suspend" :
			            pxList->cStatus == tskDELETED_CHAR ? "Deleted" : "???",
			         (int)usStackRemaining, (int)usStackDepth, (int)pxList->uxTCBNumber);
						}
						else
						{
							usStackRemaining = usTaskCheckFreeStackSpace( ( unsigned portCHAR * ) pxList->pxStack );
			         ...
			         pxList->cStatus == tskREADY_CHAR ? "Ready" : 
			          pxList->cStatus == tskBLOCKED_CHAR ? "Blocked" :
			           pxList->cStatus == tskSUSPENDED_CHAR ? "Suspend" :
			            pxList->cStatus == tskDELETED_CHAR ? "Deleted" : "???",
			         (int)usStackRemaining, (int)usUserStackRemaining, (int)usStackDepth, (int)pxList->uxTCBNumber);
						}
						uxCount++;
					}
					pxList++;
				...
				while( pxList->pxTCB != NULL )
				{
			       ...
			       pxList->cStatus == tskREADY_CHAR ? "Ready" : 
			        pxList->cStatus == tskBLOCKED_CHAR ? "Blocked" :
			         pxList->cStatus == tskSUSPENDED_CHAR ? "Suspend" :
			          pxList->cStatus == tskDELETED_CHAR ? "Deleted" : "???",
			       (int)usStackRemaining, (int)pxList->usStackDepth, (int)pxList->uxTCBNumber);
						uxCount++;
					}
					pxList++;
				}
...
static void prvInitialiseTCBVariables( tskTCB *pxTCB, unsigned portSHORT usStackDepth, const signed portCHAR * const pcName, unsigned portBASE_TYPE uxPriority )
{
  pxTCB->magic = MAGIC_KEY;
    
	pxTCB->usStackDepth = usStackDepth;

	/* Store the function name in the TCB. */
	strncpy( ( char * ) pxTCB->pcTaskName, ( const char * ) pcName, configMAX_TASK_NAME_LEN );
...
tskTCB *pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		memset( pxNewTCB, 0, sizeof( tskTCB ) );
...
/*-----------------------------------------------------------*/
	...
	static void prvListTaskWithinSingleList( void *pxListRet, xList *pxList, signed portCHAR cStatus )
	{
...
#endif
			signed portCHAR cStatus;
		} *pxListTCB = (struct listTCB *)pxListRet;
		
		while(pxListTCB->pxTCB != NULL)
			pxListTCB++;
		/* Write the details of all the TCB's in pxList into the buffer. */
		...
		do
		{
			...
			pxListTCB->pxTCB = pxNextTCB;
			pxListTCB->uxPriority = pxNextTCB->uxPriority;
			pxListTCB->pxStack = pxNextTCB->pxStack;
			pxListTCB->uxTCBNumber = pxNextTCB->uxTCBNumber;
			strncpy( ( char * ) pxListTCB->pcTaskName, ( const char * ) pxNextTCB->pcTaskName, configMAX_TASK_NAME_LEN );
			pxListTCB->usStackDepth = pxNextTCB->usStackDepth;
#if( HAVE_USP == 1 )
			pxListTCB->uxSuper =  pxNextTCB->uxSuper;
#endif
			pxListTCB->cStatus = cStatus;
			pxListTCB++;
...
#if 1
	#if portSTACK_GROWTH < 0
		portSTACK_TYPE *pucStackType = ( portSTACK_TYPE * ) ( ( ( portSTACK_TYPE ) pucStackByte  + ( sizeof(portSTACK_TYPE) - 1 ) ) & ~( sizeof(portSTACK_TYPE) - 1 ) );
    while( *pucStackType == tskSTACK_FILL_TYPE )
      pucStackType += 16;
    pucStackType -= 16;
    while( *pucStackType++ == tskSTACK_FILL_TYPE );
		usCount = (unsigned portSHORT) ( ( portSTACK_TYPE ) pucStackType - ( portSTACK_TYPE ) pucStackByte );
		usCount /= sizeof( portSTACK_TYPE );
	#else
		portSTACK_TYPE *pucStackType = ( portSTACK_TYPE * ) ( ( portSTACK_TYPE ) pucStackByte & ~( sizeof(portSTACK_TYPE) - 1 ) );
    while( *pucStackType == tskSTACK_FILL_TYPE )
      pucStackType -= 16;
    pucStackType += 16;
    while( *pucStackType-- == tskSTACK_FILL_TYPE );
		usCount = (unsigned portSHORT) ( ( portSTACK_TYPE ) pucStackByte - ( portSTACK_TYPE ) pucStackType );
		usCount /= sizeof( portSTACK_TYPE );
	#endif
#else
