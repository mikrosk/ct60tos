...
void prog_flash(unsigned char *buffer)
{
	...
	{
		for(i=0,j=0x1700;i<0x90;buffer[j+1]=modifier[i++],buffer[j]=modifier[i++],j+=2);
		for(i=0,j=0x1900;i<0x90;buffer[j+1]=shift[i++],buffer[j]=shift[i++],j+=2);
		for(i=0,j=0x1B00;i<0x90;buffer[j+1]=altgr[i++],buffer[j]=altgr[i++],j+=2);
	}
...
{
	int 					i, handle, size, flag_18f, ligne;
	long 					longueur, offset, old_offset;
	void 					*buffer;
			...
			else
			{
				...
				else
				{
					...
					while(*ptr++ == ':')
					{
						...
						if(i<0)
							break;
						if((offset >= 0x2000L && !flag_18f && ligne>1)
						 || (offset >= 0x4000L && flag_18f)
						 || (old_offset > offset))
						{
							prog_flash(buf);
							break;	
						...
						while(ptr < (unsigned char *)buffer+longueur && size>0)
						{
							...
							if((ptr2[0] & 0xC0)!=0 && !flag_18f)
							{
								for(i=(int)(&ptr2[2]-buf);i<8192;buf[i++]=0xFF,i++);
								flag_18f=1;
							}				
							ptr2+=2;	
							size--;
