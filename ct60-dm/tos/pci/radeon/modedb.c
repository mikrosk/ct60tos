		       ...
		       const struct fb_videomode *mode, unsigned int bpp)
{
    ...
    var->refresh = mode->refresh;
   	err = radeonfb_check_var(var, info);
    var->activate &= ~FB_ACTIVATE_TEST;
    return err;
}
...
//	DPRINT("radeonfb: fb_find_mode\r\n");
	total_modedb = sizeof(modedb)/sizeof(*modedb);
	/* Set up defaults */
	if(!db)
	{
//		DPRINT("radeonfb: fb_find_mode, use default modedb\r\n");
		if(resolution->used && resolution->vesa)
		{
			db = vesa_modes;
