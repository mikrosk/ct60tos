...
static int radeon_parse_monitor_layout(struct radeonfb_info *rinfo, const char *monitor_layout)
{
	...
	if(access->funcs.equal(s1, "CRT"))
		rinfo->mon1_type = MT_CRT;
	else if(access->funcs.equal(s1, "TMDS"))
		rinfo->mon1_type = MT_DFP;
	else if(access->funcs.equal(s1, "LVDS"))
		rinfo->mon1_type = MT_LCD;
	if(access->funcs.equal(s2, "CRT"))
		rinfo->mon2_type = MT_CRT;
	else if(access->funcs.equal(s2, "TMDS"))
		rinfo->mon2_type = MT_DFP;
	else if(access->funcs.equal(s2, "LVDS"))
...
void radeon_check_modes(struct radeonfb_info *rinfo, struct mode_option *resolution)
{
	 ...
	 */
	access->funcs.copymem((void *)&radeonfb_default_var,(void *)&info->var,sizeof(struct fb_var_screeninfo));
	/*
	...
	if(rinfo->mon1_type != MT_CRT && !rinfo->panel_info.valid)
	{
		...
		if(fb_find_mode(&info->var, info, &xres_yres, modedb, dbsize, NULL, 8) == 0)
		{
			rinfo->mon1_type = MT_CRT;
			goto pickup_default;
		}
		       ...
		       const struct fb_var_screeninfo *src)
{
	...
	access->funcs.copymem((void *)src, dest, sizeof(struct fb_var_screeninfo));
