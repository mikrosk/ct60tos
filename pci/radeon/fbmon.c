...
extern void mem_set(char *p, char fill, long size);
...
static void fix_edid(unsigned char *edid, int fix)
{
	...
	{
		case FBMON_FIX_HEADER:
			access->funcs.copymem(edid_v1_header,edid,8);
			break;
		case FBMON_FIX_INPUT:
			b = edid + EDID_STRUCT_DISPLAY;
			b[0] &= ~0x80;
...
static struct fb_videomode *fb_create_modedb(unsigned char *edid, int *dbsize)
{
	...
	unsigned char *block;
	int num = 0, i;
	mode = access->funcs.malloc(50 * sizeof(struct fb_videomode), 3);
	if(mode == NULL)
		return NULL;
	mem_set((char *)mode, 0, 50 * sizeof(struct fb_videomode));
 	if(edid == NULL || !edid_checksum(edid) || !edid_check_header(edid))
	{
		access->funcs.free(mode);
		return NULL;
	}
	*dbsize = 0;
	...
	*dbsize = num;
	m = access->funcs.malloc(num * sizeof(struct fb_videomode), 3);
	if(!m)
		return mode;
	access->funcs.copymem(mode, m, num * sizeof(struct fb_videomode));
	access->funcs.free(mode);
	return m;
}
...
void fb_edid_to_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	if(!(edid_check_header(edid)))
		return;
	mem_set((char *)specs, 0, sizeof(struct fb_monspecs));
...
int fb_get_mode(int flags, unsigned long val, struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	mem_set((char *)&timings, 0, sizeof(struct __fb_timings));
