...
static int
qtd_fill (struct ehci_qtd *qtd, unsigned long buf, unsigned long len,
		int token, int maxpacket)
{
	...
	qtd->hw_buf [0] = cpu_to_le32 ((unsigned long)addr);
	qtd->hw_buf_hi [0] = cpu_to_le32 ((unsigned long)(addr >> 32));
	...
	if (likely (len < count))		/* ... iff needed */
		...
		for (i = 1; count < len && i < 5; i++) {
			addr = buf;
			qtd->hw_buf [i] = cpu_to_le32 ((unsigned long)addr);
			qtd->hw_buf_hi [i] = cpu_to_le32 ((unsigned long)(addr >> 32));
			buf += 0x1000;
			if ((count + 0x1000) < len)
				count += 0x1000;
			else
...
{
	unsigned long		dma = QH_NEXT (qh->qh_dma);
	struct ehci_qh	*head;
	...
	if (!head->qh_next.qh) {
		unsigned long	cmd = readl (&ehci->regs->command);
...
)
{
	...
	if (unlikely (qh == NULL)) {
		...
		qh = qh_make (ehci, urb, 3 /* GFP_ATOMIC */);
		*ptr = qh;
	}
	if (likely (qh != NULL)) {
		struct ehci_qtd	*qtd;
