...
periodic_next_shadow (union ehci_shadow *periodic, unsigned long tag)
{
#if 0
	switch (tag) {
	case Q_TYPE_QH:
...
static void periodic_unlink (struct ehci_hcd *ehci, unsigned frame, void *ptr)
{
	...
	unsigned long			*hw_p = &ehci->periodic [frame];
...
/*	dev_dbg (&qh->dev->dev,
	...
	for (i = qh->start; i < ehci->periodic_size; i += period) {
		...
		unsigned long			*hw_p = &ehci->periodic [i];
...
iso_stream_alloc (int mem_flags)
{
	...
	stream = Mxalloc(sizeof *stream, (short)mem_flags);
	if (likely (stream != NULL)) {
		_memset ((char *)stream, 0, sizeof(*stream));
...
{
	static const unsigned char smask_out [] = { 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f };
...
//	spin_lock_irqsave (&ehci->lock, flags);
	...
	{
		stream = iso_stream_alloc(3 /* GFP_ATOMIC */);
		if (likely (stream != NULL)) {
...
iso_sched_alloc (unsigned packets, int mem_flags)
{
	...
	iso_sched = Mxalloc (size, (short)mem_flags);
	if (likely (iso_sched != NULL)) {
		_memset((char *)iso_sched, 0, size);
...
{
	unsigned	i;
	unsigned long	dma = urb->transfer_dma;
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		uframe->bufp = (buf & ~(unsigned long long)0x0fff);
		buf += length;
		if (unlikely ((uframe->bufp != (buf & ~(unsigned long long)0x0fff))))
			uframe->cross = 1;
	}
}
...
sitd_slot_ok (
	...
	unsigned long			uframe,
	struct ehci_iso_sched	*sched,
	unsigned long			period_uframes
)
{
	unsigned long			mask, tmp;
	unsigned long			frame, uf;
...
{
	unsigned long			now, start, max, period;
	int			status;
...
)
{
	...
	itd->hw_bufp [pg] |= cpu_to_le32 (uf->bufp & ~(unsigned long)0);
	itd->hw_bufp_hi [pg] |= cpu_to_le32 ((unsigned long)(uf->bufp >> 32));

	/* iso_frame_desc[].offset must be strictly increasing */
	if (unlikely (!first && uf->cross)) {
		unsigned long long bufp = uf->bufp + 4096;
		itd->pg = ++pg;
		itd->hw_bufp [pg] |= cpu_to_le32 (bufp & ~(unsigned long)0);
		itd->hw_bufp_hi [pg] |= cpu_to_le32 ((unsigned long)(bufp >> 32));
	}
}
...
//			spin_unlock_irqrestore (&ehci->lock, flags);
		...
		_memset ((char *)sitd, 0, sizeof *sitd);
