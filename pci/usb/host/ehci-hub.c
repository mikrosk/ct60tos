...
ehci_hub_descriptor (
	...
	_memset ((char *)&desc->bitmap [0], 0, (long)temp);
	_memset ((char *)&desc->bitmap [temp], 0xff, (long)temp);
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			if (temp & PORT_SUSPEND) {
				...
				writel ((temp & ~PORT_WAKE_BITS) | PORT_RESUME,
					&ehci->regs->port_status [wIndex]);
//				ehci->reset_done [wIndex] = jiffies	+ msecs_to_jiffies (20);
			}
			break;
	...
	if (status & ~0xffff)	/* only if wPortChange is interesting */
		...
		*((unsigned long *) buf) = cpu_to_le32 (status);
		break;
	case SetHubFeature:
		switch (wValue) {
		case C_HUB_LOCAL_POWER:
