...
{
	unsigned long	params = readl (&ehci->caps->hcs_params);
...
{
	unsigned long	params = readl (&ehci->caps->hcc_params);
...
dbg_status_buf (char *buf, unsigned len, const char *label, unsigned long status)
{
...
static int __attribute__((__unused__))
dbg_intr_buf (char *buf, unsigned len, const char *label, unsigned long enable)
{
	return scnprintf (buf, len,
...
static int
dbg_command_buf (char *buf, unsigned len, const char *label, unsigned long command)
{
	return scnprintf (buf, len,
...
static int
dbg_port_buf (char *buf, unsigned len, const char *label, int port, unsigned long status)
{
	char	*sig;
...
static inline char token_mark (unsigned long token)
{
	__unsigned long v = le32_to_cpu (token);
	if (v & QTD_STS_ACTIVE)
		return '*';
	if (v & QTD_STS_HALT)
		return '-';
...
)
{
	...
	temp = scnprintf (next, size,
			...
			(cpu_to_le32 (QTD_TOGGLE) & qh->hw_token)
				? "data1" : "data0",
		...
		do {
			...
			case Q_TYPE_QH:
				...
				if (temp == seen_count && p.ptr) {
					unsigned long	scratch = le32_to_cpup (
							&p.qh->hw_info1);
					struct ehci_qtd	*qtd;
					char		*type = "";
