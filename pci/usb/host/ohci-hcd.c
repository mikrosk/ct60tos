...
#include "dmapool.h"    /* needed by ohci-mem.c when no PCI */
...
//	unsigned long	flags;
	...
	urb_priv = Mxalloc (sizeof (urb_priv_t) + size * sizeof (struct td *), (short)mem_flags);
	if (!urb_priv)
		return -ENOMEM;
	_memset ((char *)urb_priv, 0, sizeof (urb_priv_t) + size * sizeof (struct td *));
...
//		spin_unlock (&urb->lock);
	...
	if (ed->state == ED_IDLE) {
		...
		if (ed->type == PIPE_ISOCHRONOUS) {
			unsigned short	frame = ohci_frame_no(ohci);
...
//	ohci->next_statechange = jiffies;
	...
	ohci->hcca = dma_alloc_coherent (sizeof *ohci->hcca, &ohci->hcca_dma, 0);
	if (!ohci->hcca)
		return -ENOMEM;
...
static int ohci_run (struct ohci_hcd *ohci)
{
	...
	// flush those writes
	(void) ohci_readl (ohci, &ohci->regs->control);
	_memset ((char *)ohci->hcca, 0, sizeof (struct ohci_hcca));
...
static irqreturn_t ohci_irq (struct usb_hcd *hcd, struct pt_regs *ptregs)
{
	...
	} else if ((ints = ohci_readl (ohci, &regs->intrstatus)) == ~(unsigned long)0) {
		disable (ohci);
...
//	flush_scheduled_work();
	...
	if (ohci->hcca) {
		dma_free_coherent (sizeof *ohci->hcca, ohci->hcca, ohci->hcca_dma);
		ohci->hcca = NULL;
		ohci->hcca_dma = 0;
	}
}
