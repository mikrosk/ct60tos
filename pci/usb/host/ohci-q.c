...
static void periodic_link (struct ohci_hcd *ohci, struct ed *ed)
{
	...
	for (i = ed->branch; i < NUM_INTS; i += ed->interval) {
		...
		unsigned long		*prev_p = &ohci->hcca->int_table [i];
		struct ed	*here = *prev;
...
static void
td_fill (struct ohci_hcd *ohci, unsigned long info,
...
{
	unsigned long	tdINFO = hc32_to_cpup (ohci, &td->hwINFO);
	int	cc = 0;
	...
	/* ISO ... drivers see per-TD length/status */
  	if (tdINFO & TD_ISO) {
 		unsigned short	tdPSW = ohci_hwPSW (ohci, td, 0);
		int	dlen = 0;
	 ...
	 */
		...
		unsigned long	tdBE = hc32_to_cpup (ohci, &td->hwBE);
...
ed_halted (struct ohci_hcd *ohci, struct td *td, int cc, struct td *rev)
{
	...
	unsigned long			toggle = ed->hwHeadP & cpu_to_hc32 (ohci, ED_C);
...
//		wmb ();
	...
	if (HC_IS_RUNNING(ohci_to_hcd(ohci)->state)
			...
			&& !ohci->ed_rm_list) {
		unsigned long	command = 0, control = 0;
