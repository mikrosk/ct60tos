...
static void finish_unlinks (struct ohci_hcd *, unsigned short, struct pt_regs *);
...
//	spin_unlock_irq (&ohci->lock);
	...
	enables = 0;
	while (temp--) {
		unsigned long stat = ohci_readl (ohci,
	...
	ohci_writel (ohci, (unsigned long) ohci->hcca_dma, &ohci->regs->hcca);
...
//	spin_lock_irqsave (&ohci->lock, flags);
	...
	/* look at each port */
	for (i = 0; i < ports; i++) {
		unsigned long	status = roothub_portstatus (ohci, i);
...
ohci_hub_descriptor (
	...
	unsigned long		rh = roothub_a (ohci);
	...
	if (rh & RH_A_NOCP)		/* no overcurrent reporting? */
	    ...
	    temp |= 0x0008;
	desc->wHubCharacteristics = (unsigned short)cpu_to_hc16(ohci, temp);
...
{
	unsigned long *portstat = &ohci->regs->roothub.portstatus [port];
	unsigned long	temp;
	unsigned short	now = ohci_readl(ohci, &ohci->regs->fmnumber);
	unsigned short	reset_done = now + PORT_RESET_MSEC;
	...
	do {
		...
		mdelay(PORT_RESET_HW_MSEC); // msleep(PORT_RESET_HW_MSEC);
...
//				schedule_work (&ohci->rh_resume);
	...
	case GetHubStatus:
		...
		*(unsigned long *) buf = cpu_to_le32 (temp);
		break;
	case GetPortStatus:
		if (!wIndex || wIndex > ports)
			goto error;
