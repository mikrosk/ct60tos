...
struct usb_hub_descriptor {
	...
	unsigned char  PortPwrCtrlMask[(USB_MAXCHILDREN + 1 + 7) / 8];
} __attribute__ ((packed));
...
struct usb_hub {
	...
	char			(*buffer)[8];
	unsigned long		buffer_dma;	/* DMA address for buffer */
	union {
...
#error event_bits[] is too short!
	...
	unsigned char			power_budget;	/* in 2mA units; or zero */
...
hub_set_power_budget (struct usb_device *hubdev, unsigned mA)
{
	...
	if(mA<500)
		hub->power_budget = mA/2;
	else
		hub->power_budget = 500/2;
}
