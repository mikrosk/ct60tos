...
//	if ((status = usb_submit_urb (hub->urb, 3 /* GFP_ATOMIC */ )) != 0
...
static inline int
hub_clear_tt_buffer (struct usb_device *hdev, unsigned short devinfo, unsigned short tt)
{
...
//	unsigned long		flags;
	 ...
	 */
	if ((clear = (struct usb_tt_clear	*)Mxalloc(sizeof *clear, 3 /* SLAB_ATOMIC */ )) == NULL) {
...
static void hub_power_on(struct usb_hub *hub)
{
	...
	mdelay(hub->descriptor->bPwrOn2PwrGood * 2); // sleep(hub->descriptor->bPwrOn2PwrGood * 2);
}
...
static void hub_activate(struct usb_hub *hub)
{
	...
	hub->quiescing = 0;
	hub->activating = 1;
	status = usb_submit_urb(hub->urb, 3 /* GFP_NOIO */ );
	if (status < 0)
	...
	struct usb_endpoint_descriptor *endpoint)
{
	...
	hub->buffer = usb_buffer_alloc(hdev, sizeof(*hub->buffer), 3 /* GFP_KERNEL */ ,	&hub->buffer_dma);
	if (!hub->buffer) {
	...
	hub->status = (void *)Mxalloc(sizeof(*hub->status), 3 /* GFP_KERNEL */ );
	if (!hub->status) {
	...
	hub->descriptor = (struct usb_hub_descriptor *)Mxalloc(sizeof(*hub->descriptor), 3 /* GFP_KERNEL */ );
	if (!hub->descriptor) {
...
//	spin_lock_init (&hub->tt.lock);
	...
	hub->urb = usb_alloc_urb(0, 3 /* GFP_KERNEL */ );
	if (!hub->urb) {
...
descriptor_error:
	...
	hub = (struct usb_hub *)Mxalloc(sizeof(*hub), 3 /* GFP_KERNEL */ );
	if (!hub) {
		dev_dbg (&intf->dev, "couldn't malloc hub struct\n");
		return -ENOMEM;
	}
...
static int hub_resume(struct usb_interface *intf)
{
	...
	for (port1 = 1; port1 <= hdev->maxchild; port1++) {
		struct usb_device	*udev;
		unsigned short			portstat, portchange;
		...
		int retry_counter)
{
	 ...
	 * value.
	 */
	for (i = 0; i < GET_DESCRIPTOR_TRIES; (++i, mdelay(100) /* msleep(100) */ )) {
		if (USE_NEW_SCHEME(retry_counter)) {
			struct usb_device_descriptor *buf;
			int r = 0;

#define GET_DESCRIPTOR_BUFSIZE	64
			buf = (usb_device_descriptor *)Mxalloc(GET_DESCRIPTOR_BUFSIZE, 3 /* GFP_NOIO */ );
			if (!buf) {
				retval = -ENOMEM;
				continue;
			}
...
check_highspeed (struct usb_hub *hub, struct usb_device *udev, int port1)
{
	...
	qual = (struct usb_qualifier_descriptor	*)Mxalloc(sizeof *qual, 3 /* SLAB_KERNEL*/ );
	if (qual == NULL)
		return;
...
//			spin_unlock_irq(&hub_event_lock);
		...
		dev_dbg(hub_dev, "state %d ports %d chg %04x evt %04x\n",
				...
				(unsigned short) hub->change_bits[0],
				(unsigned short) hub->event_bits[0]);
...
static int config_descriptors_changed(struct usb_device *udev)
{
	...
	buf = (struct usb_config_descriptor	*)Mxalloc (len, 3 /* SLAB_KERNEL */ );
	if (buf == NULL) {
