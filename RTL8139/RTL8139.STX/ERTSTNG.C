...
/* Peter Rottengatter.													*/
/*																		*/
/* Tab size 4, developed with Pure-C 1.1								*/
/************************************************************************/
...
static DRIVER	my_driver = {
	...
	((M_YEAR-1980) << 9) | (M_MONTH << 5) | M_DAY, "Thanks to Thomas Redelberger",
	NULL, NULL
};
...
{
	static char	fault[] = "*.STX: STinG extension module. To be started by STinG!\r\n";
...
static int16 cdecl my_set_state (PORT *port, int16 state)
{
	...
	if (state) {
		...
		memcp6 (ipEthPckt.eh.source,		ei_dev_addr(), 6);
		ipEthPckt.eh.type = TYPE_IP;

		/* put ethernet MAC address from ei_probe1 into ARP outgoing packets */
		memcp6 (arpEthPckt.eh.source,		ei_dev_addr(), 6);
		memcp6 (arpEthPckt.arp.src_ether,	ei_dev_addr(), 6);
...
*/
	case CTL_ETHER_GET_MAC :
		memcp6 ((uint8 *) argument, ei_dev_addr(), 6);
		break;
